...
>(
  (
    ...
    // Fallback for SSR or when not mobile (or not yet mounted)
    // This ensures the server and initial client render match for the desktop structure.
    if (!mounted && isMobile) return null; // Don't render desktop structure if it will switch to mobile sheet
    ...
    return (
      ...
      >
        ...
        >
          <div
            data-sidebar="sidebar"
            className="bg-sidebar backdrop-blur-xl flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow"
          >
            {children}
          </div>
        </div>
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2.5 overflow-hidden rounded-lg p-3 text-left outline-none ring-sidebar-ring transition-all focus-visible:ring-2 active:shadow-sm disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-9 group-data-[collapsible=icon]:!p-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 font-semibold",
  {
    variants: {
      variant: {
